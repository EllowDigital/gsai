...
export function use3DEffect<T extends HTMLElement = HTMLDivElement>(options: Use3DEffectOptions = {}) {
  const {
    intensity = 20,
    perspective = 1000,
    smooth = 0.1,
    ...
    const updatePosition = () => {
      ...
      if (ref.current) {
        ...
        ref.current.style.transform = `
          ...
          rotateX(${rotateX}deg) 
          rotateY(${rotateY}deg)
          ${scale && isHovered ? 'scale3d(1.05, 1.05, 1.05)' : ''}
        `;
  ...
  }, [intensity, perspective, smooth, scale, shadow, position, isHovered]);
